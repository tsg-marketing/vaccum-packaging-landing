...
const STORAGE_KEY = 'quiz_sidebar_traysealer_shown';

export default function QuizSidebarTraysealer() {
  const [tabVisible] = useState(true);
    ...
    <>
      {!open && tabVisible && (
        ...
        >
          ...
          <span className="text-base font-bold tracking-wide whitespace-nowrap">Подобрать трейсилер</span>
        </button>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm" onClick={() => setOpen(false)}>
          ...
          >
            <div className="flex items-center justify-between p-5 border-b">
              <h3 className="font-bold text-lg">Подберём трейсилер</h3>
              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setOpen(false)}>
                <Icon name="X" size={18} />
              </Button>
            </div>
            <div className="p-5">
              <QuizTraysealerWidget variant="modal" onClose={() => { setOpen(false); setDismissed(true); }} />
            </div>
          </div>
        </div>
      )}
