...
const CATEGORY_IDS_STR = CATEGORIES.map(c => c.id).join(',');
...
export default function TraysealerCatalog({ onInquiry }: TraysealerCatalogProps) {
  ...
  useEffect(() => {
    const CACHE_KEY = 'traysealerProductsCache_v1';
    const CACHE_TIMESTAMP_KEY = 'traysealerProductsCacheTimestamp_v1';
    ...
    const fetchProducts = async (useCache = true) => {
      ...
      try {
        const response = await fetch(`https://functions.poehali.dev/2d5f9278-9fd7-4ee8-86c0-e8b7c096608c?categories=${CATEGORY_IDS_STR}`);
  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-muted-foreground">Товары загружаются. Обновите страницу позже или оставьте заявку — пришлём подборку.</p>
